...
void il2cpp_dump() {
	...
	//cout << "dumping..." << endl;
	Logger::Info(L"il2cpp_dump", L"dumping...");
	size_t size;
	...
	for (int i = 0; i < size; ++i) {
		...
		Logger::Info(L"il2cpp_dump", L"Dumping class in %S", il2cpp_image_get_name(image));
	}
	//GallopDialog::SetDialogMessage(dialog,L"Writeing dump file...");
	std::ofstream outStream("gakumas.exe.local\\dump.cs");
	...
	outStream.close();
	Logger::Info(L"il2cpp_dump", L"dump done!");
